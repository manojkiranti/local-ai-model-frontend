...
| POST | `/v1/chat` | bearer | **Stateful.** `{session_id?, message, model?, stream?, options?}`. Omit `session_id` to start a conversation; the server owns history — send only the new message. `stream:false` → `{session_id, message:{role,content}, model}`. `stream:true` → **NDJSON** deltas (`{message:{content}, done}`, **not** SSE) and the new session id in the **`X-Session-Id` response header**. |
| POST | `/v1/agent` | bearer | **Stateful**, chat WITH tools. `{session_id?, message, model?}` → `{session_id, final_answer, stop_reason, iteration_count, error_message, trace[]}`. Non-streaming. |
| GET  | `/v1/sessions` | bearer | Sidebar list `[{id, title, created_at, updated_at, message_count}]`, newest-updated first. |
| GET  | `/v1/sessions/{id}` | bearer | Full thread `{…, messages:[{id, seq, role, content, trace, model, created_at}]}`. Agent rows have non-null `trace`; chat rows `trace=null`. 404 = gone. |
...
- **Per-conversation mode toggle** — **Chat** uses streaming `/v1/chat` (live
  token deltas); **Tools** uses `/v1/agent` (spinner, then the answer). The
  choice is remembered per conversation.
- **Agent replies** render a collapsible **"How it worked"** trace panel
  (iterations → tool calls: name / args / status / result), for both live turns
  and agent rows loaded from history (`trace != null`).
- **Files** produced by agent tools are parsed from the trace and fetched from
